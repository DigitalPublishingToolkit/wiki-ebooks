$(document).ready(function () {
    ...
    var projects = [], //will store the projects - data to be used in the buttons/modules display
        ...
        props,
        sub;
    var license = '&meta=siteinfo&siprop=rightsinfo'; //will add license info to api request
    var ready = false;
    ...
    function resetAddContent() {
        ...
        //empty contentModules and arrays
        contentModules = [];
        actionParse = [];
        actionQuery = [];

        //enable input field, set focus and empty val
        $('#w-proj').prop('disabled', false).focus().val('');
        ...
        //enable #makeQuery btn
        $('#makeQuery').prop('disabled', false);
    ...
    function makeQuery() {
        ...
        if (ready) {
            ...
            $('#makeQuery').prop('disabled', true);
            $('#w-proj').prop('disabled', true);
            ...
            for (var p = 0; p < actionParse.length; p++) {
                ...
                //updating contentModules
                var lang = domain.slice(0, domain.indexOf('.')).toUpperCase();
                var ref = proj.charAt(0).toUpperCase() + proj.slice(1);
                //update contentModules
                contentModule = ref + ' ' + lang + ' ' + page + ' ' + actionParse[p];
            ...
            //ADD LICENSE (+license)
            if (actionParse.length > 0) {
                wikiUrlParse = "https://" + domain + "/w/api.php?action=parse&page=" + page + props + "&format=json&callback=?";
            ...
            if (proj == "wikipedia" || proj == "wikimedia") {
            ...
            //console.log(props);
            //ADD LICENSE (+license)
            wikiUrlQuery = "https://" + domain + "/w/api.php?action=query&titles=" + page + props + "&format=json&callback=?";
            ...
            /*  $.when(
                  ...
                  if (queryData) {
                    ...
                    $('#editcontent ul').fadeOut("fast", function(){
                      //query will always be requested (contributors)
                      //update edit content list
                      //loop through contentModules (which will be saved in localStorate btw)
                      for(var c = 0; c < contentModules.length; c++){
                         $('#editcontent ul').append('<li>'+contentModules[c]+'</li>');
                      }
                      $('#editcontent ul').fadeIn();
                    });

                    //
                    // var pages = queryData.query.pages;
                    // console.log(pages);
                    ...
                    //
                      ...
                      //alert('not ok');
                      $('#msg').addClass('error').text('Your request has not been completed due to an error. Please try again.');
                      //ready = false;
                  }
                  if (parseData) {
            ...
            $.getJSON(wikiUrlQuery, function (queryData) {
                    console.log("success query");
                    $('#makeQuery').text('Request');

                    //
                    var pages = queryData.query.pages;
                    //console.log(pages[Object.keys(pages)[0]].contributors[1].name);
                    //alert('Last edited by: ' + pages[Object.keys(pages)[0]].revisions[0].user);
                    //check if page exists
                    console.log(pages[Object.keys(pages)[0]].pageid);

                    if (pages[Object.keys(pages)[0]].pageid > 0 || pages[Object.keys(pages)[0]].pageid != undefined) {
                        ...
                        for (var q = 0; q < actionQuery.length; q++) {
                            ...
                            var lang = domain.slice(0, domain.indexOf('.')).toUpperCase();
                            var ref = proj.charAt(0).toUpperCase() + proj.slice(1);
                            //update contentModules (check if page exists, first!!!)
                            contentModule = ref + ' ' + lang + ' ' + page + ' ' + actionQuery[q];
                            contentModules.push(contentModule);
                        }
                        ...
                        //needs to be fixed
                        $('#msg').addClass('error').css('display', 'block').text('The page you requested does not exist.');
                        //enable field and button
                        $('#w-proj').prop('disabled', false).focus();
                        $('#makeQuery').prop('disabled', false);
                    }
                ...
                .done(function () {
                    console.log("second success query");
                })
                .fail(function () {
            ...
            $.getJSON(wikiUrlParse, function () {
                    console.log("success parse");
                })
                .done(function () {
                    console.log("second success parse");
                })
                .fail(function () {
    ...
    $('#btn-addcontent').on('click', function () {
        resetAddContent();
    });
